...
export function TokenForm({ initialToken, renewalDraft, isEdit }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!propertyId) {
      setError('Please select or create an assigned Property.');
      return;
    }
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            >
              ...
              <span>New Property Inline</span>
            </button>
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">
              Select Property Location <span className="text-rose-500">*</span>
            </label>
            {loadingMasters ? (
              <div className="text-xs text-slate-400 py-2">Loading properties...</div>
            ) : (
              <select
                required
                value={propertyId}
              ...
              >
                ...
                {properties.map(pr => (
                  <option key={pr.id} value={pr.id}>
                    {pr.name} {!pr.is_active ? '(Inactive)' : ''}
                  </option>
                ))}
              </select>
            )}
            <p className="text-[11px] text-slate-500 mt-1">
              Select an existing property or click &quot;New Property Inline&quot; to save a reusable location.
            </p>
          </div>
        </div>
